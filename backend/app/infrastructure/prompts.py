...
5. 仅当素材明显较长且包含多个互不相关的独立主题时，才可输出 {"cards": [卡片对象, ...]}（最多5张，每张字段约束与单卡片完全相同）；常规素材一律输出单卡片对象"""

STRUCTURE_USER_TEMPLATE = "请将以下知识素材结构化：\n\n{content}"

CARD_IMAGE_USER_PROMPT = (
    "请识别图片中的学习内容（OCR），并按约定JSON格式输出知识卡片"
    "（title=主题标题，summary=一句话摘要，key_points=要点列表，qa_pairs=自测问答，tags=标签）。"
    "若图片中无可识别的有效内容，将 title 置为空字符串。"
)
