...
@pytest.fixture
async def kb_with_cards(session_factory, fake_redis):
    """预置用户+知识库+2张卡片，返回 {kb_id, card_ids}。"""
    async with session_factory() as db:
        user = User(email=f"map-{datetime.now().timestamp()}@t.cn", password_hash="x", created_at=_now())
        db.add(user)
        await db.flush()
        kb = KnowledgeBase(user_id=user.id, name="导图测试库", created_at=_now())
        db.add(kb)
        await db.flush()
        card_ids = []
