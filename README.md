...
| P1 | 错题本与测验 / 知识搜索 / 文档图片导入 / 导图编辑 / 复习统计 | ✅ 已完成 |
| P2 | 知识分享论坛 / 导图拼图自测 / 语义搜索 / 复习打卡 / 手机号注册 | ✅ 已完成（SMS需华为云凭据后启用） |
...
| MaaS | 大模型服务 | 结构化/导图层级提炼/语义向量 | 开通服务、创建部署任务、获取 API Key 与服务端点 |
| SMS（P2 可选） | 短信服务 | 手机号注册验证码 | 开通短信服务、创建签名与模板；演示环境可用 console 驱动免开通 |
